    ...
    switch(feature) {
        case 0x1:
            if (reg == R_ECX) {
                ...
                value |= (CPUID_EXT_DSCPL | CPUID_EXT_PDCM);
            } else if (reg == R_EDX) {
                value &= ~CPUID_PSE36;
            }
            break;
        case 0x7:
            if (reg == R_EBX) {
                ...
                value |= CPUID_7_0_EBX_FDP_EXCPTION_ONLY |
                         CPUID_7_0_EBX_FCS_FDS_DEPRE;
            } else if (reg == R_ECX) {
                value &= ~CPUID_7_0_ECX_SGX_LC;
            } else if (reg == R_EDX) {
                value |= CPUID_7_0_EDX_CORE_CAPABILITY;
            }
            break;
        case 0x40000001:
            if (reg == R_EAX) {
